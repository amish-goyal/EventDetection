...
class LDA:
    def __init__(self, K, alpha, beta, cont, docs, docs_nt, V, docs_times, output_dir, prev_lda, \
        ...
        self.phi_prob = numpy.zeros((K, V)) # word count of each topic and vocabulary
        if prev_lda != None:
...
def output_word_topic_dist(lda, voca):
    ...
    topic_word_dist = open(lda.output_dir + "/topic_distr.txt","w")
    fjs = open(lda.output_dir + '/jsdivergence.out','w')
    oldvoca_file = open(lda.output_dir + "/oldvoca.txt","w")
    newvoca_file = open(lda.output_dir + "/newvoca.txt","w")
    oldphi_prob = open(lda.output_dir + "/oldphi.txt","w")
    newphi_prob = open(lda.output_dir + "/newphi.txt","w")
    for k in range(lda.K):
	#topic_word_dist.write('\nTOPIC='+ str(k) + ' WORDS=')
	#for w in numpy.argsort(-phi[k])[:1000]: #Tanu added 
	#    topic_word_dist.write(voca[w]+" ")	
        for w in numpy.argsort(-phi[k])[:1000]:
            #print "%s: %f" % (voca[w], phi[k,w])
            topics_file.write(voca[w] + " ")
	    topic_word_dist.write(str(phi[k][w])+ " ")
        topics_file.write("\n")
        topic_word_dist.write("\n")
    
    ## Mapping topics back to the tweets
    topics = []
    topic_dist = lda.n_m_z / lda.n_m_z.sum(axis=1)[:,numpy.newaxis]
    
    tweet_topic_dist=open(lda.output_dir + '/tweet_topic_dist.pkl','wb')
    pickle.dump(topic_dist,tweet_topic_dist)
    pickle.dump(lda.n_m_z,tweet_topic_dist)
    tweet_topic_dist.close()
    
    wordTopicDist=open(lda.output_dir+'/word_topic_dist.pkl','wb')
    pickle.dump(lda.worddist(),wordTopicDist)
    wordTopicDist.close()

    with open(lda.output_dir + '/mapped_topics.txt',"w") as mapped_topics:
        for i in xrange(topic_dist.shape[0]):
            top5 = numpy.argsort(topic_dist[i])
            top5 = top5[len(top5)-5:len(top5)]   
            topics.append(top5)
            print >> mapped_topics, top5
            print >> mapped_topics, '\n\n'
